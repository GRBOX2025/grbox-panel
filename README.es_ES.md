...
```
VERSION=v1.7.9 && <(curl -Ls "https://raw.githubusercontent.com/mhsanaei/GRBOX Panel/$VERSION/install.sh") $VERSION
```
...
2. Ejecuta el comando `GRBOX Panel` en la terminal y elige `Gestión de Certificados SSL`.
...
1. Ejecuta el comando `GRBOX Panel` en la terminal y elige `Certificado SSL de Cloudflare`.
...
2. Una vez que se haya descargado el paquete comprimido, ejecuta los siguientes comandos para instalar o actualizar GRBOX Panel:
...
eliminar GRBOX Panel de docker
...
1. **Restablecer la Ruta Base Web:**
   - Abre tu terminal.
   - Ejecuta el comando `GRBOX Panel`.
...
3. **Ver Configuración Actual:**
   - Para ver tu configuración actual, utiliza el comando `GRBOX Panel settings` en el terminal o selecciona `Ver Configuración Actual` en `GRBOX Panel`.
...
1. Ejecuta el comando `GRBOX Panel` en el terminal, luego elige `Gestión de Límite de IP`.
...
- [Documentación de API](https://www.postman.com/hsanaei/GRBOX Panel/collection/q1l5l0u/GRBOX Panel)
