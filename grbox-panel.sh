...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/MHSanaei/GRBOX Panel/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_menu() {
    ...
    wget -O /usr/bin/GRBOX Panel https://raw.githubusercontent.com/MHSanaei/GRBOX Panel/main/GRBOX Panel.sh
    chmod +x /usr/local/GRBOX Panel/GRBOX Panel.sh
    chmod +x /usr/bin/GRBOX Panel
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/mhsanaei/GRBOX Panel/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    systemctl stop GRBOX Panel
    systemctl disable GRBOX Panel
    rm /etc/systemd/system/GRBOX Panel.service -f
    systemctl daemon-reload
    systemctl reset-failed
    rm /etc/GRBOX Panel/ -rf
    rm /usr/local/GRBOX Panel/ -rf

    echo ""
    echo -e "Uninstalled Successfully.\n"
    echo "If you need to install this panel again, you can use below command:"
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/mhsanaei/GRBOX Panel/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    /usr/local/GRBOX Panel/GRBOX Panel setting -username ${config_account} -password ${config_password} >/dev/null 2>&1
    /usr/local/GRBOX Panel/GRBOX Panel setting -remove_secret >/dev/null 2>&1
    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${yellow} Panel login secret token disabled ${plain}"
    echo -e "${green} Please use the new login username and password to access the GRBOX Panel panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    /usr/local/GRBOX Panel/GRBOX Panel setting -webBasePath "${config_webBasePath}" >/dev/null 2>&1
...
reset_config() {
    ...
    /usr/local/GRBOX Panel/GRBOX Panel setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(/usr/local/GRBOX Panel/GRBOX Panel setting -show true)
    if [[ $? != 0 ]]; then
    ...
    local existing_cert=$(/usr/local/GRBOX Panel/GRBOX Panel setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        /usr/local/GRBOX Panel/GRBOX Panel setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "GRBOX Panel Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "GRBOX Panel and xray stopped successfully"
        else
...
restart() {
    ...
    check_status
    if [[ $? == 0 ]]; then
        LOGI "GRBOX Panel and xray Restarted successfully"
    else
...
status() {
    systemctl status GRBOX Panel -l
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
}
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "GRBOX Panel Set to boot automatically on startup successfully"
    else
        LOGE "GRBOX Panel Failed to set Autostart"
    fi
...
disable() {
    systemctl disable GRBOX Panel
    if [[ $? == 0 ]]; then
        LOGI "GRBOX Panel Autostart Cancelled successfully"
    else
        LOGE "GRBOX Panel Failed to cancel autostart"
    fi
...
show_log() {
    ...
    0)
        ...
        ;;
    1)
        journalctl -u GRBOX Panel -e --no-pager -f -p debug
        if [[ $# == 0 ]]; then
            before_show_menu
        fi
        ;;
...
update_shell() {
    wget -O /usr/bin/GRBOX Panel -N https://github.com/MHSanaei/GRBOX Panel/raw/main/GRBOX Panel.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ ! -f /etc/systemd/system/GRBOX Panel.service ]]; then
        return 2
    fi
    temp=$(systemctl status GRBOX Panel | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    temp=$(systemctl is-enabled GRBOX Panel)
        ...
        if [ -z "$domains" ]; then
            ...
            if echo "$domains" | grep -qw "$domain"; then
                ...
                if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                    /usr/local/GRBOX Panel/GRBOX Panel cert -webCert "$webCertFile" -webCertKey "$webKeyFile"
...
ssl_cert_issue() {
    local existing_webBasePath=$(/usr/local/GRBOX Panel/GRBOX Panel setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(/usr/local/GRBOX Panel/GRBOX Panel setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    ...
    *)
    ...
    reloadCmd="GRBOX Panel restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}GRBOX Panel restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; GRBOX Panel restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
        1)
            LOGI "Reloadcmd is: systemctl reload nginx ; GRBOX Panel restart"
            reloadCmd="systemctl reload nginx ; GRBOX Panel restart"
            ;;
        2)  
            LOGD "It's recommended to put GRBOX Panel restart at the end, so it won't raise an error if other services fails"
            read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; GRBOX Panel restart): " reloadCmd
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(/usr/local/GRBOX Panel/GRBOX Panel setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(/usr/local/GRBOX Panel/GRBOX Panel setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(/usr/local/GRBOX Panel/GRBOX Panel setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(/usr/local/GRBOX Panel/GRBOX Panel setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(/usr/local/GRBOX Panel/GRBOX Panel setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    1)
        if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
            ...
            /usr/local/GRBOX Panel/GRBOX Panel setting -listenIP "${config_listenIP}" >/dev/null 2>&1
        ...
        ;;
    2)
        /usr/local/GRBOX Panel/GRBOX Panel setting -listenIP 0.0.0.0 >/dev/null 2>&1
...
show_usage() {
    echo -e "┌───────────────────────────────────────────────────────┐
│  ${blue}GRBOX Panel control menu usages (subcommands):${plain}              │
│                                                       │
│  ${blue}GRBOX Panel${plain}              - Admin Management Script          │
│  ${blue}GRBOX Panel start${plain}        - Start                            │
│  ${blue}GRBOX Panel stop${plain}         - Stop                             │
│  ${blue}GRBOX Panel restart${plain}      - Restart                          │
│  ${blue}GRBOX Panel status${plain}       - Current Status                   │
│  ${blue}GRBOX Panel settings${plain}     - Current Settings                 │
│  ${blue}GRBOX Panel enable${plain}       - Enable Autostart on OS Startup   │
│  ${blue}GRBOX Panel disable${plain}      - Disable Autostart on OS Startup  │
│  ${blue}GRBOX Panel log${plain}          - Check logs                       │
│  ${blue}GRBOX Panel banlog${plain}       - Check Fail2ban ban logs          │
│  ${blue}GRBOX Panel update${plain}       - Update                           │
│  ${blue}GRBOX Panel legacy${plain}       - legacy version                   │
│  ${blue}GRBOX Panel install${plain}      - Install                          │
│  ${blue}GRBOX Panel uninstall${plain}    - Uninstall                        │
└───────────────────────────────────────────────────────┘"
}
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│   ${green}GRBOX Panel Panel Management Script${plain}                │
