...
2. دستور `GRBOX Panel` را در ترمینال اجرا کرده و گزینه `مدیریت گواهی SSL` را انتخاب کنید.
...
1. دستور `GRBOX Panel` را اجرا و گزینه `گواهی SSL کلادفلر` را انتخاب کنید.
...
3. **مشاهده تنظیمات فعلی:**
   - استفاده از دستور `GRBOX Panel settings` یا `View Current Settings` در `GRBOX Panel`
...
1. اجرای دستور `GRBOX Panel` و انتخاب `مدیریت محدودیت IP`
2. گزینه‌های موجود:
...
- [مستندات API](https://www.postman.com/hsanaei/GRBOX Panel/collection/q1l5l0u/GRBOX Panel)
