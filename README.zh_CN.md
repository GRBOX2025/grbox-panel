...
1. 确保您的域名正确解析到服务器。
2. 在终端中运行 `GRBOX Panel` 命令，然后选择 `SSL证书管理`。
3. 您将看到以下选项：
...
1. 在终端中运行 `GRBOX Panel` 命令，然后选择 `Cloudflare SSL证书`。
...
2. 下载压缩包后，执行以下命令安装或升级 GRBOX Panel：
...
3. **查看当前设置:**
   - 要查看当前设置，请在终端中使用 `GRBOX Panel settings` 命令，或在 `GRBOX Panel` 面板中点击 `查看当前设置`。
...
1. 在终端中运行 `GRBOX Panel` 命令，然后选择 `IP 限制管理`。
2. 您将看到以下选项：
...
- [API 文档](https://www.postman.com/hsanaei/GRBOX Panel/collection/q1l5l0u/GRBOX Panel)
